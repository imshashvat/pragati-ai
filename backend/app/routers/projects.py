...
# ── MUST be declared before /{project_id} catch-all ──────────────────────────
# NOTE: FastAPI resolves routes in order of declaration. Because this router is
# registered with prefix="/projects", the path here is literally "/recommendations"
# which becomes GET /projects/recommendations. No conflict with /{project_id}.
...
def get_recommendations(
    ...
    """
    Returns all projects ranked for continuation suitability.
    Restricted to senior_official and admin only.
    ...
    # ── Fetch rank-1 (top) driver for each prediction in ONE query ────────────
    pred_ids = [p.id for p in preds]
    from app.models.prediction import RiskDriver
    top_drivers = (
        db.query(RiskDriver)
    ...
    for proj_id, proj in projects.items():
        ...
        # top_reason: reframe the top driver from risk → opportunity language
